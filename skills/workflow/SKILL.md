...
After orphan recovery, find the first `[ ]` task in priority order: 🔴 → 🟠 → 🟡 → 🟢. Skip `[!]` blocked tasks. Skip the 💭 Wishlist tier. Skip any task whose `wf/NNN-*` branch still exists (the not-orphan branches above) — that task's work is on a branch awaiting merge.
