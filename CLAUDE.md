...
7. **Wave-based parallel execution for multi-phase plans** - when an approved plan has substantially independent phases, default to spawning parallel worktree agents (`Agent({ isolation: "worktree", run_in_background: true })`) in waves rather than running phases sequentially. Sequence only when phases have hard contract dependencies (e.g., a test phase verifying contracts produced by other phases). Surface the wave structure in 2–3 sentences before starting; don't ask permission for the structure unless there's a real fork.
8. **Review each phase before merging** - when a worktree agent reports completion, spawn relevant specialized review agents (security-reviewer, ts-reviewer, test-reviewer, documentation-reviewer, dependency-reviewer, ai-security-reviewer, etc.) on its diff in parallel before merging the branch. Implementing agents' green tests prove intent matches behavior but don't catch what the agent didn't think to look for. If a reviewer flags Critical or High findings, dispatch the implementing agent back via `SendMessage` to address before merge.
